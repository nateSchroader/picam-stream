...
7. **Security-first expansion** (the current architecture): the app now **binds localhost**
   ...
   **sandboxing** (see gotcha below), a **concurrent-stream cap** (`PICAM_MAX_STREAMS` → 503),
   ...
   frame — near-zero cost), **camera controls** (`/set_controls` — flip, white balance,
   image tuning, and **JPEG quality `q`**, with a **reset-to-defaults**; all persisted
   alongside resolution), and `/healthz`. Config moved to `StateDirectory`
   (`/var/lib/picam-stream/`).
...
- `CameraManager` — owns the `Picamera2` instance. `set_resolution()`, `update_controls()`,
  and `reset_controls()` serialize (re)configuration with a lock and save state. **Flip and
  JPEG-quality (`jpeg_q`) changes go through the stop→reconfigure→start path** (the `Transform`
  and `JpegEncoder(q=…)` are fixed at configure time); other controls apply **live** via
  `set_controls()`. `_start()` reapplies persisted controls after every (re)start.
  `_load()/_save()` use `stream-config.json`; `validate_controls()` clamps/whitelists.
- `StreamingHandler` — routes: `/` & `/index.html` (UI), `/stream.mjpg` (multipart MJPEG,
  ...
  `/set_controls` (`brightness=…&hflip=1&awb=auto&quality=90`, or `reset=1`). GET on the
  setters → 405.
- Tunables/env at the top: `PORT`/`PICAM_PORT`, `BIND`/`PICAM_BIND`, `MAX_STREAMS`/
  `PICAM_MAX_STREAMS`, `ALLOWED`, `DEFAULT_SIZE`, `DEFAULT_FPS`, `MIN/MAX_FPS`,
  `DEFAULT_JPEG_Q`/`MIN/MAX_JPEG_Q`, `LIVE_CONTROLS`, `EXPOSURE_RANGE`, `GAIN_RANGE`,
  `AWB_MODES`, `STATE_DIR`/`CONFIG_PATH`.
...
- **⚠️ systemd sandboxing vs. the camera:** the unit is hardened (`ProtectSystem=strict`,
  ...
  Use **`ProtectHome=read-only`, not `=yes`**: the app's `ExecStart` lives under the user's
  home, and `=yes` hides the script so the unit can't start. Baseline
  `systemd-analyze security picam-stream` ≈ **1.9 OK** — keep it in that range.
...
- **Constraints:** 512 MB RAM; **software** JPEG encoding (CPU-bound). Both higher resolution
  **and** higher JPEG quality (`jpeg_q`, default 85) raise CPU — e.g. 1080p @ q85 ≈ 17–18 fps
  on the Zero 2 W; drop either to recover frame rate. Keep defaults modest — these lower the
  *effective* frame rate, not the sensor's. The stream cap (`PICAM_MAX_STREAMS`, default 4)
